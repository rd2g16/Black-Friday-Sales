...
def encode_best_preds(df: pd.DataFrame, feature: str):
    """Function for fitting the OneHotEncoders to the features. T test are applied on
    the categories and the ones associated with p values smaller than 0.05 are added
    to the data frame."""
    kept = get_strong_preds(df, feature)
    keys = sorted(df[feature].unique())
    encoder = OneHotEncoder(sparse=False)
    encoded = np.array(df[feature])
    encoded = encoder.fit_transform(encoded.reshape(-1,1))
    encoded = pd.DataFrame(encoded, columns=keys)

    for key in keys:
        if key in kept:
            df[f'{feature} {key}'] = list(encoded[key])
...
class AttributesAdder(BaseEstimator, TransformerMixin):
    ...
    def encode_preds(self, X: pd.DataFrame, feature: str):
        """Method similar to the encode_best_preds function. It fits the OneHotEncoders
        to the features and saves them inside the self.encoders dict, as well as all the categories
        of the features and the categories with a significant effect on the dependent variable.
        """
